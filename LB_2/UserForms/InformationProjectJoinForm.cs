...
{
    public partial class InformationProjectJoinForm : Form
    {
        ...
        public InformationProjectJoinForm()
        {
            InitializeComponent();
            connection.Open();
        }

        private void InformationProjectForm_Load(object sender, EventArgs e)
        {
        ...
        private void InformationProjectForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            connection.Close();
        }
        ...
        {
            string query_id = $"SELECT [project_id] FROM [group] WHERE [user_id]={User_id}";
            SqlCommand command = new SqlCommand(query_id, connection);
            SqlDataReader reader = command.ExecuteReader();
            string project_idasd = "";
            while (reader.Read())
            {
                project_idasd = reader[0].ToString();
            }
            reader.Close();
            if (project_idasd != "1")
            {
                MessageBox.Show("Ви вже перебуваєте в проекті. Спочатку вийдіть з нього, щоб приєднатися до інших.", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            query_id = $"SELECT COUNT([user_id]) FROM [group] WHERE [project_id]={project_id}";
            command = new SqlCommand(query_id, connection);
            reader = command.ExecuteReader();
            int NumberOfPeopleInProject = 0;
            while (reader.Read())
            {
                NumberOfPeopleInProject = Int32.Parse(reader[0].ToString());
            }
            reader.Close();

            if (NumberOfPeopleInProject > 5)
            {
                MessageBox.Show("Ви не можете приєднатися до цього проекту так як він заповнений.");
            }
            else
            {
                query_id = $"UPDATE [group] SET [project_id]={project_id} WHERE [user_id]={User_id}";
                command = new SqlCommand(query_id, connection);
                command.ExecuteNonQuery();
                MessageBox.Show("Ви успішно приєдналися до даного проекту.");
            }
