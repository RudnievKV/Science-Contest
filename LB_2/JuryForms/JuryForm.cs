...
+ "Integrated Security=true";
        ...
        public int User_id;
        private int flagWhatTableIsCurrentlyOnScreen = 2;
        public JuryForm()
        {
            InitializeComponent();
            connection.Open();
        ...
        private void JuryForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            connection.Close();
            Application.Exit();
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (flagWhatTableIsCurrentlyOnScreen == 1 || flagWhatTableIsCurrentlyOnScreen == 2)
                {
                    string queryString = $"SELECT [grade] FROM [project] WHERE [project_id]={dataGridView1.SelectedRows[0].Cells[0].Value.ToString()}";
                    SqlCommand command = new SqlCommand(queryString, connection);
                    SqlDataReader reader = command.ExecuteReader();
                    string grade = "";
                    while (reader.Read())
                    ...
                    comboBox1.SelectedIndex = comboBox1.FindStringExact(grade);
                }
            }
            catch (Exception ex)
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (flagWhatTableIsCurrentlyOnScreen == 1 || flagWhatTableIsCurrentlyOnScreen == 2)
                {
                    string queryString = $"UPDATE [project] SET [grade]={comboBox1.SelectedItem.ToString()} WHERE [project_id]={dataGridView1.SelectedRows[0].Cells[0].Value.ToString()}";
                    SqlCommand command = new SqlCommand(queryString, connection);
                    command.ExecuteNonQuery();
                    MessageBox.Show("Оцінка выставлена");
                }
                else
                {
